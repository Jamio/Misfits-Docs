...
const formatValue = value => {
  ...
  if (Math.abs(rounded) < 1000) return rounded.toLocaleString("en-GB");
  return new Intl.NumberFormat("en-GB", { notation:"compact", maximumFractionDigits:1 }).format(rounded).replace(/k/g,"K");
};
...
function renderPlayer(data, registry) {
  ...
  byId("player-detail").innerHTML = `<section class="page-head service-head"><div><div class="eyebrow">Personnel record</div><h1>${escapeHTML(person.name)}</h1></div><span class="status-tag ${person.status}">${escapeHTML(person.status)}</span></section><section class="stats-strip">${stat(formatValue(person.operations),"Operations played")}${stat(formatValue(person.authored),"Operations created")}${stat(formatDuration(person.playtime),"Time deployed")}${stat(formatValue(person.kills),"Recorded kills")}${stat(formatValue(person.deaths),"Deaths")}</section><div class="service-grid"><section class="service-panel"><h2>Combat</h2><dl>${serviceRow("Infantry kills",formatValue(person.infantryKills))}${serviceRow("Vehicle kills",formatValue(person.vehicleKills))}${serviceRow("Longest kill",formatDistance(person.longestKill))}${serviceRow("Shots fired",formatValue(person.shots))}${serviceRow("Launcher shots",formatValue(person.launcherShots))}${serviceRow("Grenades thrown",formatValue(person.grenades))}${serviceRow("Explosives placed",formatValue(person.explosives))}${serviceRow("Friendly kills",formatValue(person.friendlyKills))}</dl></section><section class="service-panel"><h2>Medical</h2><dl>${serviceRow("Treatments given",formatValue(person.treatments))}${serviceRow("Unconscious events",formatValue(person.unconscious))}${serviceRow("Damage received",formatValue(person.damageReceived))}${serviceRow("Respawns",formatValue(person.respawns))}</dl></section><section class="service-panel"><h2>Movement</h2><dl>${serviceRow("Distance on foot",formatDistance(person.distanceFoot))}${serviceRow("Distance in vehicles",formatDistance(person.distanceVehicle))}${serviceRow("Time in vehicles",formatDuration(person.vehicleTime))}${serviceRow("Last operation",person.lastOperation || "—")}</dl></section></div>${createdSection}<section class="service-history"><h2>Played operations</h2><div class="table-wrap"><table><thead><tr><th>Operation</th><th>Date</th><th>Campaign</th><th>Role</th><th>Group</th><th>Time</th><th>Kills</th><th>Deaths</th></tr></thead><tbody>${historyRows || '<tr><td colspan="8">No played operations recorded.</td></tr>'}</tbody></table></div></section>`;
}

function serviceRow(label, value) { return `<div><dt>${label}</dt><dd>${value}</dd></div>`; }

function operationPlayerRecord(player, registry) {
  const distance = value => value === undefined || value === null ? "—" : formatDistance(Number(value));
  const duration = value => value === undefined || value === null ? "—" : formatDuration(Number(value));
  return `<details class="operation-player-record"><summary><span>${escapeHTML(displayName(player,registry))}</span><span>View full record</span></summary><div class="service-grid"><section class="service-panel"><h3>Combat</h3><dl>${serviceRow("Total kills",formatRecorded(player.kills))}${serviceRow("Infantry kills",formatRecorded(player.infantryKills))}${serviceRow("Vehicle kills",formatRecorded(player.vehicleKills))}${serviceRow("Friendly kills",formatRecorded(player.friendlyKills))}${serviceRow("Longest kill",distance(player.longestKill))}${serviceRow("Shots fired",formatRecorded(player.shots))}${serviceRow("Launcher shots",formatRecorded(player.launcherShots))}${serviceRow("Grenades thrown",formatRecorded(player.grenades))}${serviceRow("Explosives placed",formatRecorded(player.explosives))}</dl></section><section class="service-panel"><h3>Medical</h3><dl>${serviceRow("Treatments given",formatRecorded(player.treatments))}${serviceRow("Unconscious events",formatRecorded(player.unconscious))}${serviceRow("Damage received",formatRecorded(player.damageReceived))}${serviceRow("Deaths",formatRecorded(player.deaths))}${serviceRow("Respawns",formatRecorded(player.respawns))}</dl></section><section class="service-panel"><h3>Movement</h3><dl>${serviceRow("Distance on foot",distance(player.distanceFoot))}${serviceRow("Distance in vehicles",distance(player.distanceVehicle))}${serviceRow("Time in vehicles",duration(player.vehicleTime))}${serviceRow("Operation time",duration(player.playtimeSeconds))}</dl></section></div></details>`;
}
